  ...
  DomElement.prototype = {
    ...
    text: function text(val) {
      if (!val) {
        ...
        var elem = this[0]
        // 过滤掉多余的标签
        var result = elem.innerHTML.replace(/<\/?[\s\S]*?\/?>/g, '')
        return result
      } else {
        // 设置 text
        return this.forEach(function (elem) {
...
*/
  ...
  function Head(editor) {
    ...
    this.droplist = new DropList(this, {
      ...
      list: [{ $elem: $('<h3>Heading</h3>'), value: '<h2>' }, { $elem: $('<p>Body</p>'), value: '<p>' }],
...
*/
  ...
  Link.prototype = {
    ...
    _insertLink: function _insertLink(text, link) {
      ...
      var regExpLink = /^http(s)?\:\/\//
      if (!link.match(regExpLink)) link = 'http://' + link
      ...
      if (checkResult === true) {
        editor.cmd.do('insertHTML', '<a href="' + link + '" target="_blank" style="color: #4990E2">' + text + '</a>')
      } else {
        alert(checkResult)
      }
    },
...
*/
  ...
  function ForeColor(editor) {
    ...
    this.droplist = new DropList(this, {
      ...
      list: [{ $elem: $('<i style="color:#000000;" class="w-e-icon-pencil2"></i>'), value: '#000000' }, { $elem: $('<i style="color:#FC5832;" class="w-e-icon-pencil2"></i>'), value: '#FC5832' }],
 ...
 */
  ...
  Hr.prototype = {
    ...
    onClick: function onClick(e) {
      ...
      // 执行 hr 命令
      editor.cmd.do('insertHTML', '<hr style=\"margin: 25px auto; border-top: 0px; background: rgb(216, 216, 216); height: 1px; width: 50%\" />')
    }
  }
...
*/
  ...
  function getPasteHtml(e, filterStyle) {
    ...
    // word带样式粘贴的过滤
    pasteHtml = pasteHtml.replace(/<html[\s\S]*head>/img, '')
    ...
    uploadImg: function uploadImg(files) {
      ...
      arrForEach(files, function (file) {
        ...
        if (/\.(jpg|jpeg|png|bmp|gif)$/i.test(name) === false) {
          ...
          customUploadImg.accept && customUploadImg.accept()
          return
        }
        if (maxSize < size) {
          // 上传图片过大
      ...
      if (onchange && typeof onchange === 'function') {
        ...
        this.change = function (compareEveryChar) {
          ...
          onChangeTimeoutId = setTimeout(function () {
            // 触发配置的 onchange 函数
            onchange(currentHtml, currentText)
